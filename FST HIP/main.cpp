...
void print_help_text(struct SearchOptions* s, struct FSTOptions* o) {
    ...
    printf("  GPU settings:\n");
    printf("    -d <device_id>\n");
    printf("         The hip device used to run the program.\n");
    printf("             Default: %d\n\n", o->hipDevice);
    printf("    -t <threads>\n");
    printf("         Number of hip threads to assign to the program.\n");
    printf("             Default: %d\n\n", o->nThreads);
...
int main(int argc, char* argv[]) {
            ...
            for (int i = c.startX; i < s.nSamplesNX; i++) {
                ...
                for (int quad = c.startQuad; quad < (s.quadMode ? 8 : 1); quad++) {
                    ...
                    if (output.hipError_t == 0x0) {
                        ...
                        if (output.hipError_t == 701) {
                            if (!o.silent) fprintf(stderr, "Error: Too many hip threads requested for the number of registers on your device.\n");
                            if (!o.silent) fprintf(stderr, "       Run this program with --help for details on how to change number of threads.\n");
                            sprintf(logContent, "Too many hip threads requested");
                            write_line_to_log_file(LOG_ERROR, logContent, logf);
                        }
                        else {
                            if (!o.silent) fprintf(stderr, "Error: Normal (%.10g, %.10g, %.10g) failed with the following error code: %d.\n", testNormal[0], testNormal[1], testNormal[2], output.hipError_t);
                            sprintf(logContent, "Normal (%s, %s, %s) failed with error code %d", float2string_max(testNormal[0], 1).c_str(), float2string_max(testNormal[1], 1).c_str(), float2string_max(testNormal[2], 1).c_str(), output.hipError_t);
                            write_line_to_log_file(LOG_ERROR, logContent, logf);
                        }

                        return output.hipError_t;
                    }
                }
    ...
    if (test_device()) {
        ...
        write_line_to_log_file(LOG_INFO, "hip Device Test Successful", logf);
    }
    else {
        if (!o.silent) printf("Error: hip device test failed.\nThis run may have encountered an error on a previous normal check.");

        write_line_to_log_file(LOG_ERROR, "hip Device Test Failed", logf);
    }
